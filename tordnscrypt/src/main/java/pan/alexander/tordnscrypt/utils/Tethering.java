...
public class Tethering {
    ...
    public String[] activateTethering(boolean privacyMode) {
        ...
        if (blockHotspotHttp) {
            ...
            blockHttpRulePreroutingTCP = iptablesPath + "iptables -t nat -A tordnscrypt_prerouting -i wlan0 -p tcp ! -d 192.168.43.0/24 --dport 80 -j RETURN";
            blockHttpRulePreroutingUDP = iptablesPath + "iptables -t nat -A tordnscrypt_prerouting -i wlan0 -p udp ! -d 192.168.43.0/24 --dport 80 -j RETURN";
        }
        ...
        if (!privacyMode) {
            if (torTethering && routeAllThroughTorTether && itpdTethering) {
                ...
                tetheringCommands = new String[]{
                        ...
                        blockHttpRulePreroutingUDP,
                        busyboxPath+ "sleep 1",
                        busyboxPath + "cat "+ appDataDir +"/app_data/tor/unlock_tether | while read var1; do "+ iptablesPath +"iptables -t nat -A tordnscrypt_prerouting -i wlan0 -p tcp -d $var1 -j REDIRECT --to-port "+ torTransPort +"; done",
                        iptablesPath + "iptables -A tordnscrypt_forward -p tcp --dport 53 -j RETURN",
                        iptablesPath + "iptables -A tordnscrypt_forward -p udp --dport 53 -j RETURN",
                        blockHttpRuleForwardTCP,
                        blockHttpRuleForwardUDP
