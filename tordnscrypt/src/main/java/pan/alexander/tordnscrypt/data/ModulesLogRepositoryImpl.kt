...
class ModulesLogRepositoryImpl : ModulesLogRepository {
    ...
    override fun getDNSCryptLog(): List<String> {
        dnsCryptLogFileReader = dnsCryptLogFileReader ?: OwnFileReader(applicationContext, "$appDataDir/logs/DnsCrypt.log")
        return dnsCryptLogFileReader?.readLastLines() ?: emptyList()
    }

    override fun getTorLog(): List<String> {
        torLogFileReader = torLogFileReader ?: OwnFileReader(applicationContext, "$appDataDir/logs/Tor.log")
        return torLogFileReader?.readLastLines() ?: emptyList()
    }

    override fun getITPDLog(): List<String> {
        itpdLogFileReader = itpdLogFileReader ?: OwnFileReader(applicationContext, "$appDataDir/logs/i2pd.log")
